		...
		<div class="wraper">
			...
			<div class="login">
				<div class="container">
					<div class="menu">
						<ul>
							<li><a class="active">Thêm tập mới</a></li>
						</ul>
					</div>
					...
					<div class="form mid">
						<form:form action = "admin/phim-newLink.htm?p=${maPhim}" method = "post">
							<div class="name">
								<p>Tập phim số: </p>
								<input type="text" placeholder="Tập phim mới" name="tap" value = "${tap}"/>
							...
							<div class="pass">
								<p>Link phim</p>
								<input type="url" placeholder="Link phim" name= "link" value = "${link}"/>
								<c:if test="${link_fail != null}">
									<p >${link_fail}</p>
								</c:if>
							</div>
							<div class="btn">
								<button style = "margin-left:30%">Thêm</button>
							</div>
						</form:form>
					</div>
				</div>
